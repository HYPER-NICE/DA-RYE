...
CREATE TABLE DELIVERY_TYPE_CODE
(
    --  기본키
    ID                   BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '배송 타입 코드의 고유 ID (기본 키)',

    -- 데이터
    NAME                 VARCHAR(50)  NOT NULL UNIQUE COMMENT '배송 타입 이름 (예: 배송, 수거)',
    DESCRIPTION          VARCHAR(255) NULL COMMENT '배송 타입 코드에 대한 상세 설명으로, 타입의 의미와 사용 목적을 설명합니다.',
    ...
    LAST_MODIFIED_MEMBER BIGINT       NULL COMMENT '레코드를 마지막으로 수정한 회원 ID',
    DELETED_DATE         DATETIME(6) DEFAULT NULL COMMENT '레코드가 삭제된 날짜 (논리 삭제)',
    CONSTRAINT FK_DELIVERY_TYPE_CODE_MEMBER FOREIGN KEY (LAST_MODIFIED_MEMBER) REFERENCES MEMBER (ID) ON UPDATE CASCADE ON DELETE RESTRICT
)
    COMMENT = '배송 타입 코드를 저장하며, 배송의 형태를 관리하는 테이블입니다. 예를 들어 배송, 수거 등의 타입을 포함합니다.';
...
(
    ...
    ORDER_TYPE_CODE_ID    BIGINT       NOT NULL COMMENT '주문 유형 코드 ID',
    ...
    CONSTRAINT FK_ORDER_DELIVERY_MAIN_TYPE FOREIGN KEY (ORDER_TYPE_CODE_ID) REFERENCES DELIVERY_TYPE_CODE (ID) ON UPDATE CASCADE ON DELETE RESTRICT,
    UNIQUE KEY (ORDER_MAIN_ID, ORDER_TYPE_CODE_ID),
...
(
    -- 기본키
    ID                  BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '주문 결제 상세 고유 ID (기본 키)',

    -- 외래키
    ORDER_PAYMENT_MAIN_ID        BIGINT       NOT NULL COMMENT '주문 결제 상세 고유 ID (기본 키)',
    ...
    UNIQUE (ORDER_PAYMENT_MAIN_ID, PAYMENT_METHOD_CODE_ID, ORDER_PAYMENT_STATUS_CODE_ID),
